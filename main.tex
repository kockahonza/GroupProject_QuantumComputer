...
\begin{abstract}
        Quantum computing has the potential to transform many sectors, including and not limited to cybersecurity, drug development and weather forecasting by transforming computing power. 
        In recent years quantum computing has made the transition from scientific curiosity and theoretical study to very small, but physical, computers being built.
        Also while taking the step from university research to corporate research labs which funded by governments and large companies such as IBM and Google.\cite{boerkamp_quantum_2022}
        In this light, this paper has described in detail two methods of building a quantum computer. 
        One using trapped ions which is a method that has successfully built many very small scale quantum computers. The other is linear optical quantum computing which is a very attractive contender for large scale quantum computers in the future. 
        Trapped ion systems use ions in a single trap and the qubit is encoded as the valence electron's state. 
        Linear optical quantum computing uses photons and the qubit is encoded as its spatail state. 
\end{abstract}
