...
\paragraph{The Cirac and Zoller CNOT gate \cite{ciracQuantumComputationsCold1995}} This design is specifically for an array of ions confined in a linear Paul trap.
...
If we then use an adequately tuned light source at one of the ions so that it couples to the internal states and the lowest collective motional we can achieve entanglement between the two.
The theoretical calculation of the tuning can be found in the paper, but in short the frequency needs to be detuned by $f_x$ from an internal state and the ion position should be at a node of the light wave.
As we have an array of N atoms let us use the kets $\ket{0}_n$ and $\ket{1}_n$ to refer to the nth atom's internal state and the kets \kz and \ko to refer to the collective motional state.
